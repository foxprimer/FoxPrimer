...
This module is used to determine which genomes are available for ChIP primer
design, and to install genomes when needed.
...
before  'ucsc_genomes'  =>  sub {
    ...
    unless ( $self->has_ucsc_genomes ) {
        $self->_set_ucsc_genomes($self->_define_ucsc_genoems);
    }
};
